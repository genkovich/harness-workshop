...
**Ця гілка:** `step-02-messages` — результат етапу 02.
...
Очікуємо 8 тестів без мережі. Повний npm test запускай на готовому рішенні. Живий API: npm start після налаштування ключа. Починаючи з етапу 02, передай задачу явно: npm start -- "Перевір списання клієнта 42.". Без задачі програма завершується з помилкою до HTTP-запиту.

**Наступна тема:** [03. Функція запиту](runbooks/03-function.md).
