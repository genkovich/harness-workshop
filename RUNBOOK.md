...
**Пройти цикл по рядках у дебагері:**

```bash
node --inspect-brk --import tsx --env-file-if-exists=.env src/main.ts
```

Процес зупиниться до виконання коду; це очікування дебагера, не зависання API.
У VS Code відкрий команду **Debug: Attach to Node Process** й обери цей процес.
Постав breakpoint після generateText, перед runTool і після додавання tool result.
Продовжуй виконання й дивись reply.toolCalls, call.toolCallId та messages.
Спочатку ти бачиш рішення моделі, потім результат нашої функції, потім нову історію.
Живий прогін у дебагері також потребує ключа. Ctrl+C у терміналі завершує процес.
