# 02. Повідомлення

[Усі теми](README.md) · [Попередня](01-model.md) · [Наступна](03-function.md)

**Перед початком:** код після `step-01-model`. **Результат теми:** `step-02-messages`. Змінюємо лише `src/`.

## Що робимо й навіщо

Відокремлюємо правила system від задачі user. Масив messages стане історією: згодом ми додамо сюди запити моделі на тули й результати.

### Що означає type ModelMessage

`ModelMessage` із пакета `ai` описує повідомлення: роль user, assistant або tool і допустимий вміст. `import type` потрібен TypeScript для перевірки коду; під час виконання це не окремий обʼєкт і не запит до моделі.

### Що таке системний промпт

Поле system задає правила поведінки моделі на час запиту. User містить конкретну задачу людини. На цьому етапі правило лише одне: відповідати українською. Коли введемо агента news у темі 04, розкладемо його правила за роллю, метою, діями та обмеженнями. Текст промпту не виконує функції й не перевіряє дозволи: для цього потрібен код.

## Чого бракує зараз і що зміниться

Один prompt змішує все в одному тексті. Нам треба окремо передавати задачу людини й правила, а пізніше зберігати відповіді та результати дій. Для цього вводимо messages.

- process.argv[2] читає задачу з командного рядка; trim прибирає зайві пробіли.
- Перевірка !task не дає запустити запит без задачі. Тему й бажаний результат задає людина у команді.
- role: user позначає задачу користувача. system задає правила поведінки на час запиту.
- Після зміни задачу можна змінити командою запуску, не редагуючи код.

## Маленькі зміни

У `src/main.ts` додай ModelMessage до імпорту з ai:

```ts
import { generateText, type ModelMessage } from 'ai';
```

`process.argv` — аргументи запуску: елемент `[0]` містить шлях до Node, `[1]` — до програми, `[2]` — нашу задачу. `?.trim()` прибирає пробіли, лише якщо аргумент існує. Без аргументу отримаємо `undefined`, із самими пробілами — порожній рядок; `!task` ловить обидва випадки. `process.exit(1)` завершує програму з кодом помилки до звернення до моделі.

Після const model додай задачу та початкове повідомлення:

```ts
const task = process.argv[2]?.trim();
if (!task) {
  console.error('Помилка: передай задачу. Наприклад: npm start -- "Знайди до трьох обговорень про harness engineering і coding agents за останні 7 днів. Прочитай коментарі та збережи український дайджест із посиланнями."');
  process.exit(1);
}
```

Порожній ввід має завершити програму до HTTP-запиту. `ModelMessage[]` означає масив повідомлень. У першому обʼєкті `role` вказує автора, `content` містить текст задачі. Після перевірки створюємо повідомлення:

```ts
const messages: ModelMessage[] = [{ role: 'user', content: task }];
```

У generateText прибери поле prompt. На його місце встав:

```ts
system: 'Відповідай українською.',
messages,
```

Перед викликом моделі можеш подивитися, що передаєш:

```ts
console.log('Повідомлення:', messages);
```

У команді `npm start -- "Твоя задача"` подвійний дефіс передає решту аргументів нашій програмі. Лапки тримають всю задачу одним аргументом, навіть якщо в ній є пробіли.

Окремо перевір відсутню задачу та пробіли:

```bash
npm start
npm start -- "   "
```

В обох випадках очікуємо «Помилка: передай задачу» й код завершення 1. Тема не підставляється автоматично: її треба явно назвати у задачі.

## Перевірка

```bash
npm run check
npm test -- --test-name-pattern "^0[0-2] "
npm start -- "Знайди до трьох обговорень про harness engineering і coding agents за останні 7 днів. Прочитай коментарі та збережи український дайджест із посиланнями."
```

**Автоматична перевірка:** 8 тестів без мережі. Усі тести вже є в [test/harness.test.mjs](../test/harness.test.mjs) та [test/runbooks.test.mjs](../test/runbooks.test.mjs). Число на початку назви тесту відповідає етапу; команда запускає цей і попередні етапи.

**Очікуємо:** Одна задача user і відповідь моделі. Тулів ще немає; текст про дискусії не означає, що дані перевірені.

**Якщо не так:** Якщо тип role не підходить, звір ModelMessage[]. Власну задачу передай через npm start -- "Твоя задача".

**Збережи свою зміну:**

```bash
git add src
git diff --cached
git commit -m "Етап 02: Повідомлення"
```

## Якщо не встиг: готова точка й наступна тема

Ця гілка містить **результат теми 02**. Збережи свою спробу й створи робочу гілку від готового коду:

```bash
git add src
git diff --cached
git diff --cached --quiet || git commit -m "Моя спроба етапу 02"
git fetch origin
git switch -c work-03 origin/step-02-messages
npm run check
npm test -- --test-name-pattern "^0[0-2] "
```

Власний коміт залишився у попередній гілці. Якщо work-03 вже існує, обери нове імʼя, наприклад work-03-retry. .env і node_modules залишаються на місці. Відкрий [ранбук 03](03-function.md) в тому самому редакторі: усі ранбуки й тести доступні в кожній гілці.

Якщо завершив самостійно, продовжуй у своїй гілці за наступним ранбуком; брати готовий код необовʼязково.

## Готовий код

Очікуваний вміст змінених файлів після теми. Інші файли залишаються без змін.

<details>
<summary>src/main.ts</summary>

```ts
import { generateText, type ModelMessage } from 'ai';
import { groq } from '@ai-sdk/groq';

const model = groq(process.env.GROQ_MODEL || 'qwen/qwen3.8-27b');

const task = process.argv[2]?.trim();
if (!task) {
  console.error('Помилка: передай задачу. Наприклад: npm start -- "Знайди до трьох обговорень про harness engineering і coding agents за останні 7 днів. Прочитай коментарі та збережи український дайджест із посиланнями."');
  process.exit(1);
}
const messages: ModelMessage[] = [{ role: 'user', content: task }];

const reply = await generateText({
  model,
  system: 'Відповідай українською.',
  messages,
  maxRetries: 0,
  maxOutputTokens: 1200,
  abortSignal: AbortSignal.timeout(60_000),
});

console.log('Причина завершення:', reply.finishReason);
console.log('Відповідь:', reply.text);
```

</details>

