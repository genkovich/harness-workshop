...
## А якщо впав Hacker News?

`maxRetries` у `generateText` стосується API моделі. Він не повторює наш `fetch` до HN.

- **HN відповів 503:** можна зачекати й один раз повторити GET.
- **HN відповів 404 або повтор не допоміг:** повертаємо помилку через `executeTool`, щоб модель могла змінити дію.

Короткий приклад для пояснення, **не додатковий обовʼязковий крок**. У контрольній гілці HN-помилка одразу повертається через `executeTool`.

Якщо хочеш додати один повтор у `src/news/api.ts`, імпортуй паузу й назви її тривалість:
...
У функції `get` заміни перший виклик `fetch` на:
...
`503` означає тимчасову недоступність сервісу. Це один повтор читання; наявну перевірку `!response.ok` нижче залиш. Якщо повтор теж невдалий, вона передасть помилку в `executeTool`. Мережевий виняток чи таймаут також підуть у цей обробник без повтору. Запис файла так автоматично не повторюємо.
