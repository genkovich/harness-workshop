...
`reply.response.messages` — повідомлення, які SDK сформував із відповіді моделі. Зберігаємо повний `assistant`, включно з tool calls. Якби зберегли лише `reply.text`, втратили б запит на дію. `.filter(...)` відбирає повідомлення автора assistant, а `...` передає їх у `push` окремими елементами.
...
Наприклад, якщо модель прислала `toolCallId: 'call_1'`, результат пошуку теж має отримати `'call_1'`. Одна назва `searchStories` недостатня: модель може викликати пошук кілька разів із різними аргументами.
