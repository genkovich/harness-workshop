...
Зараз у терміналі має бути лише вміст `AGENTS.md`. Це ще не запит моделі: перевіряємо свою функцію без API.
...
Повтори команду перевірки `loadContext` із попереднього кроку. Після `AGENTS.md` тепер має зʼявитися `Rule: sources.md` і текст цього правила. Так бачимо, що змінилося після додавання rules.
...
У Claude Code файли `CLAUDE.md` з робочої теки й батьківських тек завантажуються на старті. Вкладений `src/news/CLAUDE.md` підтягується, коли агент читає файл із цієї теки. Правила `.claude/rules/` без `paths` завантажуються одразу; з `paths` — коли читається файл, шлях якого відповідає шаблону. [Документація завантаження інструкцій](https://code.claude.com/docs/en/memory#how-claude-md-files-load).

При `Edit` потрібно ще вирішити, чи дозволено зміну. Це робота permissions і перевірок перед тулом, наприклад `PreToolUse`. Текстові rules не є перевіркою дозволу. [Документація hooks](https://code.claude.com/docs/en/hooks#pretooluse).

Наш `loadContext` простіший: один раз читає кореневий `AGENTS.md` та всі `.md` безпосередньо в `rules/`. Вкладеного пошуку й вибору за `paths` ми не реалізовуємо. Усе, що передаємо моделі, можна простежити в кількох рядках коду.
