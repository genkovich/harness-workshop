...
`?` у `maxSteps?: number` означає необовʼязкове поле. `agent.maxSteps ?? 10` бере число з налаштувань, а якщо воно не задане — 10. Один крок тут дорівнює **одному виклику моделі**, хоча у відповіді може бути кілька тулів. Це інше обмеження, ніж `maxRetries`: retries повторювали б невдалий HTTP-запит, а кроки продовжують роботу з новою історією.
...
`return` завершує всю `runAgent`, а не лише поточний оберт. Тому `return final` лишаємо для відповіді без дій, а `return tool-result` прибираємо: після тула має відбутися наступний запит. Якщо всі дозволені кроки витрачені, повертаємо `reason: 'limit'`; виконані раніше дії при цьому не скасовуються.
