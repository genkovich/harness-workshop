...
[Усі теми](README.md) · [Попередня](07-history.md) · [Наступна](08b-limits.md)
...
Якщо отримав 429 від Groq, збережи цей код і переходь до [окремого кроку 08б](08b-limits.md). Там додамо обмежені повтори тимчасово невдалого запиту; тут спочатку перевіряємо сам цикл без мережі.
...
git fetch origin
git switch -c work-08b origin/step-08-loop
npm run check
npm test -- --test-name-pattern "^0[0-8] "
```

Власний коміт залишився у попередній гілці. Якщо work-08b вже існує, обери нове імʼя, наприклад work-08b-retry. .env і node_modules залишаються на місці. Відкрий [ранбук 08б](08b-limits.md) в тому самому редакторі: усі ранбуки й тести доступні в кожній гілці.
